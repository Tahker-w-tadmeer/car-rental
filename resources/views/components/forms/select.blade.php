...
<div {{ $attributes }} x-data='{ selected: "{{ old($name) }}", options: @json($options), open: false, search: "{{ $options[old($name)] ?? '' }}" }'
    ...
    <div class="relative mt-2">
        <input @keydown.enter.prevent="selected=Object.entries(options).filter(([key, value]) => value.toLowerCase().includes(search.toLowerCase()))[0][0] ?? null; open=false"
               @focus="open=true"
        ...
        <select tabindex="-1" name="{{ $name }}" id="{{ $id }}-select" class="hidden" x-model="selected">
            @foreach($options as $key => $option)
                <option value="">-</option>
                <option {{ $key == old($name) ? "selected" : "" }} value="{{ $key }}">{{ $option }}</option>
            @endforeach
        </select>
        <button tabindex="-1" @click="open=!open" type="button" class="absolute inset-y-0 right-0 flex items-center rounded-r-md px-2 focus:outline-none">
